...
def avg_time_per_genre_spotify(cur):
    ...
    cur.execute(
        "SELECT A.genre, AVG(S.length) FROM spotify_songs AS S JOIN spotify_artists AS A WHERE S.artist_id=A.artist_id GROUP BY A.genre ORDER BY AVG(S.length) ASC"
    )
    rows = cur.fetchall()
...
def avg_time_per_genre_itunes(cur):
    ...
    cur.execute(
        "SELECT A.genre, AVG(S.length) FROM itunes_songs AS S JOIN itunes_artists AS A WHERE S.artist_id=A.artist_id GROUP BY A.genre ORDER BY AVG(S.length) ASC"
    )
    rows = cur.fetchall()
